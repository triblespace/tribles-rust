...
impl<const KEY_LEN: usize, O: KeyOrdering<KEY_LEN>, S: KeySegmentation<KEY_LEN>> From<Leaf<KEY_LEN, O, S>> for Head<KEY_LEN, O, S> {
    fn from(head: Leaf<KEY_LEN, O, S>) -> Self {
        unsafe { transmute(head) }
    }
}

impl<const KEY_LEN: usize, O: KeyOrdering<KEY_LEN>, S: KeySegmentation<KEY_LEN>> Leaf<KEY_LEN, O, S> {
...
impl<const KEY_LEN: usize, O: KeyOrdering<KEY_LEN>, S: KeySegmentation<KEY_LEN>> HeadVariant<KEY_LEN, O, S> for Leaf<KEY_LEN, O, S> {
    fn count(&self) -> u32 {
        1
    }
