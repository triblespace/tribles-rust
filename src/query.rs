...
pub struct CursorIterator<const MAX_DEPTH: usize> {
    mode: ExplorationMode,
    depth: usize,
    key: [u8; MAX_DEPTH],
    branch_points: ByteBitset,
    branch_state: [ByteBitset; MAX_DEPTH],
    cursor: Rc<dyn ByteCursor>,
}

impl<const MAX_DEPTH: usize> CursorIterator<MAX_DEPTH> {
    pub fn new(cursor: Rc<dyn ByteCursor>) -> Self {
        Self {
...
impl<const MAX_DEPTH: usize> Iterator for CursorIterator<MAX_DEPTH> {
...
struct Query<C: VariableConstraint> {
    ...
    binding: Binding,
    iterators: [Option<CursorIterator<32>>; 256],
    stack: [u8; 256],
    depth: u8,
}

const INIT: Option<CursorIterator<32>> = None;
