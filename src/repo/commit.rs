...
use crate::{
    blob::{schemas::simplearchive::SimpleArchive, Blob}, prelude::valueschemas::Handle, query::find, trible::TribleSet, value::Value
};
...
pub fn commit(content: Blob<SimpleArchive>, signing_key: SigningKey, msg: &str, parents: impl IntoIterator<Item = Value<Handle<Blake3, SimpleArchive>>>) -> TribleSet {
    let handle = content.get_handle();
    let signature = signing_key.sign(&content.bytes);

    let commit_entity = crate::id::rngid();

    let mut commit = repo::entity!(&commit_entity,
    {
        short_message: msg,
        content: handle,
