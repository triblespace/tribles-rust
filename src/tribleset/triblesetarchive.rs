...
pub trait Universe {
    ...
    fn access(&self, pos: usize) -> Option<Value>;
...
where C: IBuild + IAccess + NumVals + Serializable {
    ...
    fn access(&self, pos: usize) -> Option<Value> {
        ...
        v[0..8].copy_from_slice(&(self.columns[0].access(pos)?.to_be_bytes()));
        v[8..16].copy_from_slice(&(self.columns[1].access(pos)?.to_be_bytes()));
        v[16..24].copy_from_slice(&(self.columns[2].access(pos)?.to_be_bytes()));
        v[24..32].copy_from_slice(&(self.columns[3].access(pos)?.to_be_bytes()));

        Some(v)
    }

    fn search(&self, v: &Value) -> Option<usize> {
        (0..=self.columns[0].num_vals()-1).binary_by(|p| self.access(p).unwrap().cmp(v)).ok()
        ...
        let r0 = (0..=self.columns[0].num_vals()-1).binary_all(|p| self.columns[0].access(p).unwrap().cmp(&v0));
        if r0.is_empty() { return None;}

        let r1 = (r0.start..=r0.end-1).binary_all(|p| self.columns[1].access(p).unwrap().cmp(&v1));
        if r1.is_empty() { return None;}

        let r2 = (r1.start..=r1.end-1).binary_all(|p| self.columns[2].access(p).unwrap().cmp(&v2));
        if r2.is_empty() { return None;}

        let r3 = (r2.start..=r2.end-1).binary_all(|p| self.columns[3].access(p).unwrap().cmp(&v3));
