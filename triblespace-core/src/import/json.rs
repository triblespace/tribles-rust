    ...
    IdGenerator: FnMut() -> ExclusiveId,
{
    ...
    pub fn import_value(&mut self, value: &JsonValue) -> Result<(), JsonImportError> {
        match value {
            JsonValue::Object(object) => {
                ...
                self.import_object(&root, object)?;
            }
    ...
    fn import_object(
        ...
        for (field, value) in object {
            self.import_field(entity, field, value)?;
        }
        Ok(())
    }
    ...
    fn import_field(
        ...
        match value {
            ...
            JsonValue::Bool(flag) => {
                ...
                self.data.insert(&Trible::new(entity, &attr.id(), &encoded));
                Ok(())
            }
            ...
            JsonValue::Array(elements) => {
                for element in elements {
                    self.import_field(entity, field, element)?;
                }
                Ok(())
            }
            JsonValue::Object(object) => {
                ...
                self.data.insert(&Trible::new(entity, &attr.id(), &value));
                self.import_object(&child, object)
            }
        }
    }
}
    ...
    Hasher: HashProtocol,
{
    ...
    pub fn import_value(&mut self, value: &JsonValue) -> Result<(), JsonImportError> {
        match value {
            JsonValue::Object(object) => {
                let _root = self.import_object(object)?;
            }
            JsonValue::Array(elements) => {
                for element in elements {
                    let JsonValue::Object(object) = element else {
    ...
    fn import_object(
        ...
        for (field, value) in object {
            self.collect_field(field, value, &mut pending)?;
        }

        let entity = Self::derive_id(&pending);
        for (attribute, value) in pending {
            ...
            let trible = Trible::new(&entity, &attribute_id, &encoded);
            self.data.insert(&trible);
        }
    ...
    fn collect_field(
        ...
        field: &str,
        value: &JsonValue,
        pending: &mut Vec<(RawId, RawValue)>,
    ) -> Result<(), JsonImportError> {
        match value {
            JsonValue::Null => Ok(()),
            JsonValue::Bool(flag) => {
                ...
                pending.push((attr.raw(), encoded.raw));
                Ok(())
            }
            JsonValue::Number(number) => {
                let attr = self.number_attribute(field);
            ...
            JsonValue::Array(elements) => {
                for element in elements {
                    self.collect_field(field, element, pending)?;
                }
                Ok(())
            }
            JsonValue::Object(object) => {
                let attr = self.genid_attribute(field);
                let child = self.import_object(object)?;
                let value = GenId::value_from(&child);
                pending.push((attr.raw(), value.raw));
                Ok(())
            }
        }
    }
